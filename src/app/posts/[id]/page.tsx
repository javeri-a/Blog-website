...
export default  function DynamicPage(){
    ...
    useEffect(() => {
      if (id) {
        const foundData = cardData.find((item) => item.id === Number(id)); 
...
return (
  <div className="px-4 sm:px-10 md:px-20 py-6 bg-gray-50">
    <div className="flex flex-col sm:flex-row gap-x-4 mb-6">
      <h1 className="font-bold text-xl sm:text-2xl">{data?.category}</h1>
      <h2 className="text-sm sm:text-base text-gray-600">{data?.date}</h2>
    </div>

    <h3 className="text-2xl sm:text-3xl font-extrabold py-4">{data?.title}</h3>
    ...
    <Image
      ...
      height={200}
      width={400}
      className="w-full sm:w-3/4 md:w-full rounded-lg mx-auto"
    />

    <h4 className="mt-4 text-sm sm:text-base md:text-lg pr-3">{data?.content}</h4>
    <BlogPage/>
  </div>
);
